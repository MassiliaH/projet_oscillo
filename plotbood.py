...
class Transfert(object):
    ...
    """crée des signaux sinusoïdaux sur la voie 1 et 2 d'amplitude donnée , de fréquence variable et offeset nulle,
            de phase nulle.
    ...
    def dephasage_gain(self,amp):
        liste_phase = []
        liste_gain = []
        liste_freq = [10**i for i in np.linspace(1,4,20)]            
        ...
        for f in liste_freq:
            ...
            #Gain du Fonction de transfert = module de rapport des amplitudes
            ...
            # position = np.argmax(abs(np.fft.fft(oscillo.get_curve(1))[1:-10])) + 1 
            
         
            #Liste la FT de sortie entrée 
            B_w = np.fft.fft(self.oscillo.get_curve(2))
            A_w = np.fft.fft(self.oscillo.get_curve(1))
            
            phase = np.angle(B_w[position]/A_w[position])
        ...
        grid(True, which='both')
        subplot(2,1,2)
        semilogx(liste_freq, liste_phase)#*1/(2*np.pi)*360)
        ylabel(u'Phase [°]')
        xlabel(u'frequence $\omega$ en [Hz]')
        grid(True, which='both')              
